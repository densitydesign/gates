...
function nodeActive(a) {
	...
	if (groupByDirection) {
		...
		(size>0)? f=f.concat(createList(mutual)) : f.push("No mutual citations<br>");
		size=Object.size(incoming);
		f.push("<h2>Incoming (" + size + ")</h2>");
		(size>0)? f=f.concat(createList(incoming)) : f.push("No incoming citations<br>");
		size=Object.size(outgoing);
		f.push("<h2>Outgoing (" + size + ")</h2>");
		(size>0)? f=f.concat(createList(outgoing)) : f.push("No outgoing citations<br>");
	} else {
